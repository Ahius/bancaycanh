  ...
  return (
    <div className="group relative rounded-lg overflow-hidden shadow-md hover:shadow-lg transition duration-300 h-96">
      ...
      <div className="absolute top-4 left-4 z-10">
        <span className="inline-block bg-white/90 text-green-700 text-xs font-bold px-3 py-1 rounded-full">
          PHONG THỦY
        </span>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6">
        <div className="bg-white/90 backdrop-blur-sm rounded-lg p-5 shadow-sm">
          <h3 className="text-lg font-bold text-gray-800 mb-2 leading-tight">
            <Link to={`/news/${newsItem.id}`} className="hover:text-green-600 transition">
              {newsItem.title}
            </Link>
          </h3>
